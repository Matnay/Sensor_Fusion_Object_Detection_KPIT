...
combined = concatenate([model_image.output, model_lidar.output])
...
fusion = Model(inputs=[model_lidar.input, model_image.input], outputs=z)
...
training_generator = JoinedGenerator(train_it, train_lidar)
validation_generator = JoinedGenerator(val_it, val_lidar)
test_generator = JoinedGenerator(test_it, test_lidar)
...
val=fusion.predict([image_mod.reshape(-1,150,150,3),image_mod_lidar.reshape(-1,150,150,3)])
print(val)
if(val[0][0]<0):
    print("car")
else:
...
image_mod_lidar=cv2.imread("5.jpg")
image_mod_lidar=cv2.resize(image_mod_lidar,(150,150))
val=fusion.predict([image_mod.reshape(-1,150,150,3),image_mod_lidar.reshape(-1,150,150,3)])
print(val)
if(val[0][0]<0):
