...
class Fusion():
 	def __init__(self):
		...
		self.radar_sub=rospy.Subscriber("radar_image",Image,self.radar_cb)
		...
		self.input_radar=keras.Input(shape=(self.IMG_HEIGHT,self.IMG_WIDTH,3))
		...
		self.w=Conv2D(16, 3, padding='same', activation='relu')(self.input_radar)
		self.output_radar=MaxPooling2D()(self.w)

		self.model_radar = keras.Model(inputs=self.input_radar,outputs=self.output_radar, name='radar_input')
		self.model_radar.summary()

		self.combined = concatenate([self.model_image.output, self.model_lidar.output, self.model_radar.output])
		...
		self.z=Dense(256, activation='relu')(self.z)
		self.z=Dense(128, activation='relu')(self.z)
		self.z = Dense(1, activation="linear")(self.z)
		self.fusion = Model(inputs=[self.model_lidar.input, self.model_image.input, self.model_radar.input], outputs=self.z)
	...
	def radar_cb(self,msg):
		self.image_radar=np.zeros((600,600,3))
		self.image_radar=self.bridge.imgmsg_to_cv2(msg)
		self.prediction()

	def prediction(self):
		image_mod=cv2.resize(self.image,(150,150))
		image_mod_lidar=cv2.resize(self.image_lidar,(150,150))
		image_mod_radar=cv2.resize(self.image_radar,(150,150))
		val=self.fusion.predict([image_mod.reshape(-1,150,150,3),image_mod_lidar.reshape(-1,150,150,3),image_mod_radar.reshape(-1,150,150,3)])
		if(val[0][0]>0):
		    print("person")
		else:
		    print("car")
